        ...
        9111: (Oe, G, j) => {
                    ...
                    const C = u => (d, c) => {
                        ...
                        Q = t(ce, {
                            ...
                            set(u, d, c, E) {
                                ...
                                let X = c;
                                if (c instanceof Promise) c.then(_ => {
                                    c.status = "fulfilled", c.value = _, m(["resolve", [d], _])
                                }).catch(_ => {
                                    c.status = "rejected", c.reason = _, m(["reject", [d], _])
                                });
                                else {
                                    !U.has(c) && s(c) && (X = h(c));
                                    const _ = !B.has(X) && U.get(X);
                                    _ && ((u, d) => {
            ...
            const y = S({
                ...
                $ = {
                    ...
                    setConfig(e) {
                        var t, s;
                        fe.initialize(), L.setChains(e.chains), L.setIsAuth(!!e.enableAuthMode), L.setIsCustomMobile(!(null == (t = e.mobileWallets) || !t.length)), L.setIsCustomDesktop(!(null == (s = e.desktopWallets) || !s.length)), v.setModalVersionInStorage(), Object.assign(F, e)
                    }
                };
